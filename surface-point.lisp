  ...
  (with-slots ((tri triangle-ref)) surface-point
    (macrolet ((truth-xor (a b)
      ...
      (let* ((normal (normal tri))
             (in-dot (dot in-direction normal))
             (out-dot (dot out-direction normal)))
...
(defmethod next-direction ((surface-point surface-point) in-direction)
  (with-slots ((tri triangle-ref)) surface-point
    (let* ((reflectivity (reflectivity tri))
           (reflectivity-mean (* (dot reflectivity (vec3 1.0 1.0 1.0))
                                 (/ 1.0 3.0))))
      (if (< (random 1.0) reflectivity-mean)
          (let* ((color (vector* reflectivity (/ 1.0 reflectivity-mean)))
                 ...
                 (normal (normal tri))
                 (tangent (tangent tri)))
            (when (minusp (dot normal in-direction))
              (vector- normal))
